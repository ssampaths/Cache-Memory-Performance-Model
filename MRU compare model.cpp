// MRU conpare model.cpp : Cache Model for MRU compare approach
//
...
int main()
{
	...
	fopen_s(&outdata_file, "MRU_probe.dat", "w");
	...
	for (i = 1; i < num_trace_addr; i++)
	{
		...
		for (w = 0; w < way; w++)\
		{
			...
			current_block = LRU_array[*set_ptr][w]; // MRU
			if (tag_RAM[*set_ptr][current_block]->v)
			{
				cmp_res = tag_compare(tag_bits, tag_RAM[*set_ptr][current_block]->tag);
				printf("\nTag compare: %d\n", cmp_res);
			}
			num_probe++;
			if (cmp_res)
